# Recado 的 NixOS 模块。
#
# 搬运自上游 github:Allenyou1126/recado 的 nix/module.nix，只改了包的默认取值：
# 上游用 flake 把 `services.recado.package` 注入进来，而 NUR 只暴露
# `legacyPackages` / `overlays` / `modules`，所以这里直接从本仓库的
# `pkgs/recado` 取包（见下面的 `packages`），不需要额外套一层 overlay。
#
# 与上游 docs/deployment.md 的承诺保持一致：
#
...
#
# 最小用法（包由本模块从 NUR 仓库自带，不需要设置 package / cliPackage）：
#
#   services.recado = {
#     enable = true;
#     settings = {
...
let
  ...
  # 本仓库自带的包。上游靠 flake 注入，NUR 这边自己取：
  # 同一份 `pkgs/recado` 就是 flake 的 `packages.<system>.recado` /
  # overlay 里的 `pkgs.allenyou-nur.recado`，三者完全一致。
  packages = pkgs.callPackage ../pkgs/recado { };
...
in
{
  options.services.recado = {
    ...
    package = lib.mkOption {
      ...
      default = packages.recado;
      defaultText = lib.literalExpression "allenyou-nur.recado";
      description = ''
        服务端包。默认取本 NUR 仓库的 `recado`，通常无需设置；
        想自行指定（例如换成上游 flake 的产物）时用 {option}`services.recado.package` 覆盖。
      '';
    };

    cliPackage = lib.mkOption {
      type = lib.types.package;
      default = packages.recado-cli;
      defaultText = lib.literalExpression "allenyou-nur.recado-cli";
